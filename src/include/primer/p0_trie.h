...
class TrieNodeWithValue : public TrieNode {
 ...
 public:
   ...
   */
  TrieNodeWithValue(char key_char, T value) : TrieNode(TrieNode(key_char)) {
    value_ = value;
    is_end_ = true;
  }
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for (int i = 0; i < static_cast<int> (key.size()) - 1; i++) {
      ...
      if (!current->HasChild(ch)) {
        std::unique_ptr<TrieNode> new_node = std::make_unique<TrieNode>(ch);
        current->InsertChildNode(ch, std::move(new_node));
      }
      current = current->GetChildNode(ch)->get();
    }
    char ch = key.back();
    if (current->HasChild(ch)) {
      if (current->GetChildNode(ch)->get()->IsEndNode()) {
        latch_.WUnlock();
        return false;
      }
      auto child_ptr = current->GetChildNode(ch);
      auto convert_node = std ::make_unique<TrieNodeWithValue<T>>(std::move(**child_ptr), value);
  ...
  bool Remove(const std::string &key) {
    ...
    for (char ch : key) {
      ...
      path.emplace_back(std ::make_pair(ch, current));
      current = current->GetChildNode(ch)->get();
    }
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    if (current->IsEndNode()) {
      auto *node = dynamic_cast<TrieNodeWithValue<T> *>(current);
      if (node != nullptr) {
        *success = true;
